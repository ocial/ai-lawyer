...
inspector_id="asst_hqlynIvo4levYoFPkhNLilJq"
judge_id="asst_tAPivWneC42ZSSEXIdndiNsy"
...
if prompt:
    ...
    if '[사건]' in last_message:
        ...
        run = client.beta.threads.runs.create(
            thread_id=thread_id,
            assistant_id=judge_id
        )
        with st.spinner('결과를 작성하는 중...'):
            while run.status != "completed":
                time.sleep(0.2)
                run = client.beta.threads.runs.retrieve(
                    thread_id=thread_id,
